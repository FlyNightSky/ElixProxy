...
app.get('/Elix', async (req, res) => {
  try {
    ...
    if (contentType.includes('text/html')) {
      ...
      // Rewrite <link> tags for font files
      $('link[rel="stylesheet"][href*=".woff"], link[rel="stylesheet"][href*=".woff2"], link[rel="stylesheet"][href*=".ttf"], link[rel="stylesheet"][href*=".otf"]').each((_, element) => {
        const link = $(element);
        const href = link.attr('href');
        if (href) {
          const absoluteUrl = new URL(href, url).href;
          link.attr('href', `/ElixFonts?url=${encodeURIComponent(absoluteUrl)}`);
        }
      });
...
// Font proxy endpoint
app.get('/ElixFonts', async (req, res) => {
  try {
    const fontUrl = req.query.url;
    const fontResponse = await axios.get(fontUrl, { responseType: 'arraybuffer' });

    res.set('Content-Type', fontResponse.headers['content-type']);
    res.send(fontResponse.data);
  } catch (error) {
    console.error('Error retrieving the font:', error);
    res.status(500).send('Error retrieving the font.');
  }
});
